...
node 'dev-box' {
	...
	exec { 'backup pg_hba.conf':
		command => '/bin/cp -p /etc/postgresql/9.*/main/pg_hba.conf /etc/postgresql/9.*/main/pg_hba.backup',
	...
	exec { 'change pg_hba.conf':
		command => '/bin/sh -c "sed -i \'86i\local      all     osp_admin               md5\' /etc/postgresql/9.*/main/pg_hba.conf"',
		before => Exec['reload config'],
	}
