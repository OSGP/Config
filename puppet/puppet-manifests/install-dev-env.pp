...
node 'dev-box' {
	...
	exec { 'wget tomcat7':
		command => '/usr/bin/wget -q -P /home/dev/Downloads - http://apache.hippo.nl/tomcat/tomcat-7/v7.0.61/bin/apache-tomcat-7.0.61.tar.gz',
		before => Exec['wget postgresql jdbc','unpack tomcat7','change permissions of tomcat7 conf files'],	
		returns => [0, 4],
	}

	exec { 'unpack tomcat7':
		command => '/bin/tar xzf /home/dev/Downloads/apache-tomcat-7.0.61.tar.gz -C /home/dev/Downloads',
		before => Exec['wget postgresql jdbc','change permissions of tomcat7 conf files'],
	}

	exec { 'change permissions of tomcat7 conf files':
		command => '/bin/chmod 644 /home/dev/Downloads/apache-tomcat-7.0.61/conf/*',
	}

	# A JDBC is needed by Tomcat7 to connect to PostgreSQL.
	exec { 'wget postgresql jdbc':
		command => '/usr/bin/wget -q -P /home/dev/Downloads/apache-tomcat-7.0.61/lib - https://jdbc.postgresql.org/download/postgresql-9.3-1103.jdbc3.jar',
		returns => [0, 4],
	}
