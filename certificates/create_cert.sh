...
echo ########################################################################
openssl ca -config $OPENSSL_CONF -days 1095 -md sha256 -extensions $CERT_TYPE -batch -passin pass:$CA_PASS -out "$CERTS/$COMMON_NAME.cert.pem" -infiles "$REQUESTS/$COMMON_NAME.req.pem"
